...
- 原文链接: <https://www.waylandz.com/ai-agent-book/第03章-工具调用基础/>
- Part: 2
- 类型: chapter
...
![工具调用完整流程](/book-images/tool-calling-workflow.svg)
...
![多轮工具调用](/book-images/multi-round-tool-calling.svg)
...
**案例**：给了 15 个工具，其中 3 个都能搜索（web\_search、google\_search、bing\_search），LLM 不知道用哪个。
...
1. ToolMetadata（name, description, category, rate\_limit, timeout\_seconds）
2. ToolParameter 列表（source\_currency, target\_currency, amount）
