...
- 原文链接: <https://www.waylandz.com/ai-agent-book/第25章-安全执行/>
- Part: 8
- 类型: chapter
...
![WASI沙箱架构](/book-images/wasi-sandbox.svg)
...
![WASI 执行架构](/book-images/wasi-execution-flow.svg)
...
1. **consume\_fuel(true)**：启用指令计量，防止 CPU 滥用
2. **epoch\_interruption(true)**：启用超时中断，防止代码无限运行
3. **memory\_guard\_size(64MB)**：内存越界时触发页错误，而不是默默溢出
...
3. **spawn\_blocking**：WASM 执行可能阻塞，必须放在独立线程
...
5. **异步隔离**：使用 spawn\_blocking 避免阻塞异步运行时
